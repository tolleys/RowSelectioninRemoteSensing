...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
#####JCEager20 Yield 
JCEager20_Yield <- read_excel("/scratch/bell/tolleys/UAV_DATA/MLPrediction/JCEager20/20JCEager_EndofSeason.xlsx", sheet=5)[-1,c(1,5,32)]
#JCEager20_Yield <- read_excel("~/Documents/Purdue/PhD/Research/Chp3_RowSelection/DATA_Processing/Yield/20JCEager_EndofSeason.xlsx", sheet=5)[-1,c(1,5,32)]
JCEager20_Yield <- na.omit(JCEager20_Yield)
head(JCEager20_Yield)


JCEager20 <- read_excel("/scratch/bell/tolleys/UAV_DATA/MLPrediction/JCEager20/RowSelection_AllData_sat.xlsx", sheet=4, na = "NA")
...
JCEager20 <- JCEager20 %>%
  select(grep("Plot", names(JCEager20)),
         grep("RowSegment", names(JCEager20)),
         grep("Trim", names(JCEager20)),
         grep("CC_", names(JCEager20)),
         grep("canopy_cover_10", names(JCEager20)),
         grep("height_95", names(JCEager20)),
         grep("plot_volume", names(JCEager20)),
         grep("NDVI", names(JCEager20)))

JCEager20 = JCEager20[,!grepl("GNDVI",names(JCEager20))]; JCEager20 = JCEager20[,!grepl("mRE",names(JCEager20))]
...
RS1234 <- JCEager1234 %>%
  group_by(Plot) %>%
  summarise_at(vars(CC_20200617:NDVI_20200826), mean, na.rm = TRUE)
...
save.image(file='/scratch/bell/tolleys/UAV_DATA/MLPrediction/JCEager20/JCEager20_svmRadial_500repeats3folds_workspace.RData')
#save.image(file='~/Documents/Purdue/PhD/UAV_Data/DATA_Processing/MLPrediction_Workspaces/JCEager20_svmRadial_500repeats3folds_workspace.RData')
